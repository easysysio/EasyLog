...
// filters stack and show as removable chips. Clicking a timeline bar pins the
// dashboard to that bar's period (?from=&to=), which the timeline then buckets
// inside, so a click can narrow again. The range and filters compose into one
// shared SQL WHERE clause; all values are bound as parameters.
// =============================================================================
...
pub(crate) fn render(
    ...
    let (range_options, chips, window_chip) = furniture(&filter, base, &range);
    ...
    let timeline: Vec<Bar> = series
        ...
        .map(|(epoch, label)| {
            ...
            Bar {
                ...
                count,
                css: String::new(),
                href: filter.with_window(epoch, end).href(base),
                label,
                ts_epoch: epoch,
            }
        })
...
fn furniture(filter: &Filter, base: &str, range: &str)
-> (Vec<RangeOpt>, Vec<Chip>, Option<super::WindowChip>) {
    ...
    let window_chip = filter.window().map(|(from, to)| super::WindowChip {
        ...
        remove: filter.without_window().href(base),
    });
