...
// panels. The time range (?range=), the window pinned by clicking a timeline bar
// (?from=&to=) and every drill-down filter compose into one parameterized WHERE
// clause.
// =============================================================================
...
pub(crate) fn render(
    ...
    let (range_options, chips, window_chip) = furniture(&filter, spec, base, &range);
...
fn furniture(filter: &Filter, spec: &Spec, base: &str, range: &str)
